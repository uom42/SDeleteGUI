...
namespace SDeleteGUI.Core.SDelete
{
	internal class SDeleteManager
	{
		...
		public SDeleteManager(Func<FileInfo> binaryPathSpecifer)
		{
			...
			var knownBinPath = uom.AppTools.AppSettings.Get_string(C_LAST_KNOWN_SDELETE_NAME, string.Empty);
			...
			if (knownBinPath == null || !File.Exists(knownBinPath))
			{
				...
				if (fiUserBinPath.Exists) knownBinPath = fiUserBinPath.FullName;
			}
			...
			uom.AppTools.AppSettings.Save(C_LAST_KNOWN_SDELETE_NAME, SDeleteBinary.FullName);
