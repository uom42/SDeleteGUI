	...
	internal partial class frmMain : Form
	{
		...
		private async Task _Load()
		{
			...
			try
			{
				...
				uom.AppTools.AppSettings.Save("LastRun", DateTime.Now);
		...
		private void _Shown()
		{
			...
			this.e_StartAutoupdateOnShown("https://raw.githubusercontent.com/uom42/SDeleteGUI/master/UpdaterXML.xml");
		}
		...
		private async Task FillDisksList()
		{
			...
			RadioButton? oldSource = _optSources.FirstOrDefault(radio => radio.Checked);
			...
			{
				RadioButton furstAvailRadio = _optSources.First(radio => radio.Enabled)!;
		...
		private static async Task FillDisks_Log(RadioButton opt, ComboBox cbo, SourceModes cm, Logger log)
		{
			...
			try
			{
				...
				LogDisk? newSelection = (oldSelectedDisk == null)
					? disks.LastOrDefault()
					: (disks.FirstOrDefault(d => d.DiskLetter == oldSelectedDisk.DiskLetter)) ?? disks.LastOrDefault();
		...
		private static async Task FillDisks_Phy(RadioButton opt, ComboBox cbo, SourceModes cm, Logger log)
		{
			...
			try
			{
				...
				Win32_DiskDrive? newSelection = (oldSelectedDisk == null)
					...
					: (disks.FirstOrDefault(d => d == oldSelectedDisk))
					?? disks.FirstOrDefault();
