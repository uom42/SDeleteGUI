...
#pragma warning restore CS8765 // Nullability of type of parameter doesn't match overridden member (possibly because of nullability attributes).
	...
	internal static partial class AppInfo
	{
		...
		{
			string resManagerBasePath = $"{LocalizedStringsManager_Namespace}.{LocalizedStringsManager_ResourcesPath}";
			return new System.Resources.ResourceManager(resManagerBasePath, Assembly);
		}
		);
		...
		internal static System.Resources.ResourceManager CreateResourceManagerFromAssemblyEntryPoint(string resourcePath)
			=> new System.Resources.ResourceManager($"{Assembly.EntryPoint!.DeclaringType!.Namespace}.{resourcePath}", Assembly);
...
#pragma warning restore CS8604 // Possible null reference argument.
		...
		internal static class Extensions_StringAndFormat
		{
			...
			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			internal static (string Left, string Right)? e_Split2(this string source, int leftPartLen)
			{
			...
			/// <summary>Split string to left and right parts</summary>
			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			internal static (string Left, string Right, int SplitCharIndex)? e_Split2(this string source, char separatorChar, bool excludeSeparatorChar)
			{
				if (source == null) return null;
...
# End Region
		...
		internal static partial class Extensions_IO
		{
			...
			const string BAK_EXT = "bak";

			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			public static DirectoryInfo e_MoveToArhive(this FileInfo file, string arhiveFilesExt = BAK_EXT)
			{

				string sTimestamp = DateTime.Now.e_ToFileName();
				FileInfo fiBackup = new($"{file.FullName}.{sTimestamp}.{arhiveFilesExt}");
			...
			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			public static void e_ClearArhiveOldFiles(this DirectoryInfo arhive, string arhiveFilesExt, int maxArhiveFileCount)
			{
		...
		internal static class Extensions_Network
		{
			...
			/// <example>
			/// <code>
			/// !!! Don't use for any arifmetic calculations! Use only for Saving/Restoring IP.
			/// </code>
			/// For Math calculations with IP use <see cref="e_ToUInt32CalculableOrder"/> instead!!!
			/// </example>
			/// </summary>
			/// <returns>for 192.168.1.1 => [192,168,1,1]</returns>
			[Obsolete("!!! Don't use for any arifmetic calculations! Use only for Saving/Restoring IP")]
			[MethodImpl(MethodImplOptions.AggressiveInlining)]
...
#pragma warning restore SYSLIB0014 // Type or member is obsolete
			...
			{
				var t1 = Extensions_DebugAndErrors.e_tryCatchAsync(() => Dns.GetHostEntry(IP), cancel: cancel);
				var t2 = Extensions_DebugAndErrors.e_tryCatchAsync(() => IP.e_SendARP(), cancel: cancel);
				try
				{
					if (cancel != null)
					{
#if NET
						await Task.WhenAll(t1, t2).WaitAsync(cancel.Token);
#else
						await Task.WhenAll(t1, t2).e_WaitAsync(cancel);
#endif
				...
				return (t1.Result.Result?.HostName, t2.Result.Result);
			}
		...
		internal static partial class Extensions_DebugAndErrors
		{
			...
			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			public static void e_trycatchOLD(this Func<uint> operation, string errorMessageTemplate, params object[] messageArgs)
			{
				const uint S_OK = 0;
			...
			#region e_tryCatch Func


			/// <summary>Exec FUNC. Return result</summary>
			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			public static (T? Result, Exception? Error) e_tryCatch<T>(this Func<T> func, T? defaultValue = default, Action? finallyAction = null)
			{
				try { return (func.Invoke(), null); }
				catch (Exception ex) { return (defaultValue, ex); }
			...
			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			internal static async Task<(T? Result, Exception? Error)> e_tryCatchAsync<T>(this Func<T> func, T? defaultValue = default, CancellationTokenSource? cancel = null, Action? finallyAction = null)
			{
				try
				{
					if (cancel != null)
			...
			/// <inheritdoc cref="e_tryCatch" />
			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			internal static async Task<(T? Result, Exception? Error)> e_tryCatchAsync<T>(this Task<T> tsk, T? defaultValue = default, Action? finallyAction = null)
			{
