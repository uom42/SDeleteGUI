...
namespace uom.controls.Extenders
{
	...
	internal static class Common
	{
		...
		internal abstract class LinkedPropertiesBase
		{
			public Component AttachedObject;

			public Control? AttachedControl => AttachedObject as Control;

			public LinkedPropertiesBase(Component attachedComponent) { AttachedObject = attachedComponent; }
		}
	...
	internal class UrlClickHandler : Common.ControlExtenderBase
	{
		...
		private class LinkedProperties : Common.LinkedPropertiesBase
		{
			public ClickActionModes ClickAction;
			public string ClickActionUrl;

			public LinkedProperties(Component attachedComponent) : base(attachedComponent)
			{
				ClickAction = ClickActionModes.OpenUrlInBrowser;
				ClickActionUrl = C_DEFAULT_URL;
			}
		}

		public UrlClickHandler() : base() { }
		...
		private void ResetClickAction(Component ctl) => SetClickAction(ctl, ClickActionModes.OpenUrlInBrowser);
		...
		private void OpenUrl(LinkedProperties prop)
		{
			switch (prop.ClickAction)
			{
				...
				case ClickActionModes.OpenFolderInExplorer:
					{
						prop.ClickActionUrl.e_OpenExplorer(
							prop.ClickAction == ClickActionModes.ShowFileInExplorer
...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
	...
	internal class CheckBoxEnabler : Common.ControlExtenderBase
	{
		...
		private void OnSource_PropChanged(Control ctlSourceCheck)
		{
			...
			//else
			{
				//targetEnabledState = sourceEnabled;
			}



			//if (!sourceEnabled)				sourceChecked = false;
			//else if (prop.InversedState) sourceChecked = !sourceChecked;




			/*
			if (prop.TrackSourceEnableInverse) sourceCheckEnabled = !sourceCheckEnabled;

			if (sourceCheckEnabled)
			{
				switch (ctlSourceCheck)
				{
					case CheckBox chk: targetEnabledState = chk.Checked; break;
					case RadioButton opt: targetEnabledState = opt.Checked; break;
					default: return;
				}
				//if (prop.InversedState) targetEnabledState = !targetEnabledState;
			}
			else
			{
				targetEnabledState = sourceCheckEnabled;
			}




			if (prop.TrackSourceEnableState && !sourceCheckEnabled)
			{
				targetEnabledState = sourceCheckEnabled;
			}
			else
